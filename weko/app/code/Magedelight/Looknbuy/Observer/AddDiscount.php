...
class AddDiscount implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ((($address->getAddressType() == 'shipping' && !$quote->isVirtual()) || ($quote->isVirtual() && $address->getAddressType() == 'billing')) && $quote->getLookDiscountAmount() > 0) {
            ...
            $address->setDiscountAmount($discountAmount - $quote->getLookDiscountAmount());
            $address->setBaseDiscountAmount($baseDiscountAmount - $quote->getBaseLookDiscountAmount());

            $address->setGrandTotal($grandTotal - $quote->getLookDiscountAmount());
            $address->setBaseGrandTotal($baseGrandTotal - $quote->getBaseLookDiscountAmount());
        }
