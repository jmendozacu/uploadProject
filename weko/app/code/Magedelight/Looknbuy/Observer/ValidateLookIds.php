...
class ValidateLookIds implements ObserverInterface
{
    protected $logger;
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($quote->getId()) {
            ...
            if (count($lookIds) > 0) {
                ...
                $qtyArrays = $this->_objectManager->create('Magedelight\Looknbuy\Model\Looknbuy')->calculateProductQtys($lookIds);
                ...
                foreach ($lookIds as $lookId) {
                    ...
                    foreach ($quoteQtys as $productId => $qty) {
                        if (isset($qtyArrays[$productId][$lookId])) {
                            if ($qtyArrays[$productId][$lookId] <= $qty) {
                                $quoteQtys[$productId] -= $qtyArrays[$productId][$lookId];
                            } else {
                                $isValid = false;
                            }
                        }
                ...
                $quote->setData('look_ids', $idString);
            }
        }
