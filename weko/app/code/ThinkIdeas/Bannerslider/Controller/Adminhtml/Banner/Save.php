...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            $this->dataPersistor->set('aw_bannerslider_banner', $data);
            if ($id) {
    ...
    private function prepareData(array $data)
    {
        ...
        if ($data['page_type'] == PageType::PRODUCT_PAGE) {
            if (isset($data['rule']['bannerslider'])) {
                $conditionArray = $this->convertFlatToRecursive($data['rule'], ['bannerslider']);
                if (is_array($conditionArray['bannerslider']['1'])) {
                    $data['product_condition'] = $this->conditionConverter
                        ->arrayToDataModel($conditionArray['bannerslider']['1']);
                } else {
