...
class Url extends AbstractHelper
{
    ...
    public function seofyUrl($url)
    {
        ...
        {
            $routeUrl = $this->scopeConfig->getValue('amshopby_root/general/url', ScopeInterface::SCOPE_STORE);
        }
    ...
    protected function initialize($url)
    {
         ...
         */
        $key = $this->scopeConfig->getValue('amshopby_root/general/url', ScopeInterface::SCOPE_STORE);
    ...
    protected function cutReplaceExtraShopby($url)
    {
        ...
        if ($cut) {
            if ($this->isBrandFilterActive) {
                $key = trim($this->scopeConfig->getValue('amshopby_brand/general/url_key', ScopeInterface::SCOPE_STORE));
                $url = $key . $url;
            }
        }
        return $url;
    ...
    public function removeCategorySuffix($url)
    {
        ...
        if ($this->coreRegistry->registry('amasty_shopby_root_category_index') && $this->query)
        {
            $shopbyRootUrl = $this->scopeConfig->getValue('amshopby_root/general/url', ScopeInterface::SCOPE_STORE);
            if (strlen($suffix)) {
                $p = strrpos($shopbyRootUrl, $suffix);
                if ($p !== false && $p == strlen($shopbyRootUrl) - strlen($suffix)) {
                    return $url;
                }
            }
        }
